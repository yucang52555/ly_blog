                                        ...
                                        <div class="col-lg-4 col-sm-8">
                                            <div class="input-group">
                                                ...
                                                <span class="input-group-btn">
                                                    <button class="btn btn-default " type="button" onclick="lyblog.layerModal('/admin/attachments/select?id=userAvatar','<@spring.message code="common.js.all-attachment" />')"><@spring.message code='common.btn.choose' /></button>
                                                </span>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function saveUser(option) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.code==1){
                        lyblog.showMsgAndReload(data.msg,'success',1000);
                    }else{
        ...
        function changPass() {
            ...
            if(beforePass==""||newPass==""||reNewPass==""){
                lyblog.showMsg("<@spring.message code='common.js.info-no-complete' />",'info',2000);
                return;
            }
            if(newPass!=reNewPass){
                lyblog.showMsg("<@spring.message code='admin.user.profile.form.password.no-same' />",'error',2000);
                return;
            }
