...
public class CommentController extends BaseController {
    ...
    public JsonResult replyComment(@RequestParam("commentId") Long commentId,
        ...
        try {
            ...
            comment.setCommentAuthorUrl(LyblogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()));
    ...
    class EmailToAuthor extends Thread {
        ...
        @Override
        public void run() {
            if (StrUtil.equals(LyblogConst.OPTIONS.get(BlogPropertiesEnum.SMTP_EMAIL_ENABLE.getProp()), TrueFalseEnum.TRUE.getDesc()) && StrUtil.equals(LyblogConst.OPTIONS.get(BlogPropertiesEnum.COMMENT_REPLY_NOTICE.getProp()), TrueFalseEnum.TRUE.getDesc())) {
                if (Validator.isEmail(lastComment.getCommentAuthorEmail())) {
                    ...
                    mailService.sendTemplateMail(
                            lastComment.getCommentAuthorEmail(), "您在" + LyblogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()) + "的评论有了新回复", map, "common/mail_template/mail_reply.ftl");
                }
            }
        }
    }
                ...
                try {
                    if (status == 1 && Validator.isEmail(comment.getCommentAuthorEmail())) {
                        ...
                        mailService.sendTemplateMail(
                                ...
                                "您在" + LyblogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()) + "的评论已审核通过！", map, "common/mail_template/mail_passed.ftl");
                    }
                } catch (Exception e) {
