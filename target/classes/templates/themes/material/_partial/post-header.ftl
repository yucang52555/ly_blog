...
<#if options.theme_material_scheme?default('Paradox') == "Isolation">
    <div class="post-header_info without-thumbnail">
        <!-- Author Avatar & Name -->
        <img src="${user.userAvatar?default('/material/source/img/avatar.png')}" class="avatar-img" width="44px" height="44px" alt="${user.userDisplayName?default('lyblog')}'s avatar">
        <span class="name-span">${user.userDisplayName?default('lyblog')}</span>
    </div>
</#if>